...
export function ReceiptWizardPage() {
  ...
  const stepCompletion = [
    Boolean(supplier.trim() && warehouseName.trim() && receiptDate && invoiceCode.trim()),
    lines.length > 0,
  ...
  const focusIssue = (issue: string) => {
    const target = issue.includes('nhà cung cấp') ? { step: 0, id: 'receipt-supplier' } : issue.includes('kho nhận') ? { step: 0, id: 'receipt-warehouse' } : issue.includes('ngày nhập') ? { step: 0, id: 'receipt-date' } : issue.includes('hóa đơn') ? { step: 0, id: 'receipt-invoice' } : issue.includes('ít nhất một SKU') ? { step: 1, id: 'receipt-content' } : { step: 2, id: 'receipt-content' }
  ...
  return <div className="space-y-6"><Link to={ROUTES.warehouseReceipts} className="inline-flex items-center gap-2 text-sm font-semibold text-text-600"><ArrowLeft className="h-4 w-4" /> Danh sách phiếu nhập</Link><WarehousePageHeader eyebrow="Nhập kho" title={currentId || 'Tạo phiếu nhập kho'} description="Lưu nháp bất cứ lúc nào; tồn kho chỉ tăng sau bước xác nhận cuối." /><ProgressStepper steps={STEPS} currentStep={step} />
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px]"><section id="receipt-content" tabIndex={-1} className="rounded-md border border-surface-400 bg-white p-5 focus-visible:shadow-focus">
      {step === 0 && <div className="grid gap-4 md:grid-cols-2"><label className="text-sm font-semibold">Nhà cung cấp<input id="receipt-supplier" value={supplier} onChange={(event) => setSupplier(event.target.value)} className="input-gaming mt-2 w-full" /></label><label className="text-sm font-semibold">Kho nhận<select id="receipt-warehouse" value={warehouseName} onChange={(event) => setWarehouseName(event.target.value)} className="input-gaming mt-2 w-full"><option>Kho trung tâm TP.HCM</option><option>Kho Hà Nội</option></select></label><label className="text-sm font-semibold">Ngày nhập<input id="receipt-date" type="date" value={receiptDate} onChange={(event) => setReceiptDate(event.target.value)} className="input-gaming mt-2 w-full" /></label><label className="text-sm font-semibold">Mã hóa đơn/chứng từ<input id="receipt-invoice" value={invoiceCode} onChange={(event) => setInvoiceCode(event.target.value)} className="input-gaming mt-2 w-full" /></label><label className="text-sm font-semibold md:col-span-2">Ghi chú<textarea value={notes} onChange={(event) => setNotes(event.target.value)} rows={4} className="input-gaming mt-2 w-full" /></label></div>}
