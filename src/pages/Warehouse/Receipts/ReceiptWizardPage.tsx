...
import { SkuSelectorDialog } from '@/components/warehouse/SkuSelectorDialog'
...
export function ReceiptWizardPage() {
  ...
  const [evidenceUrls, setEvidenceUrls] = useState<string[]>(existing?.evidenceUrls ?? [])
  ...
  const [viewingSerialLineId, setViewingSerialLineId] = useState<string | null>(null)
  ...
  const viewingLine = lines.find((line) => line.id === viewingSerialLineId)
  ...
  const stepCompletion = [
    Boolean(supplier.trim() && receiptDate && invoiceCode.trim()),
    lines.length > 0,
    lines.length > 0 && getReceiptValidationIssues(receipt, store.variants, store.serials).length === 0,
    true,
  ]
  ...
  const addVariant = (variantId: string) => {
    ...
    setLines((current) => [...current, { id: crypto.randomUUID(), variantId, quantity: 1, unitCost: 0, serials: [], putawayLocation: '' }]); setSkuSelectorOpen(false); setMessage('Đã thêm SKU vào phiếu.')
  }
  ...
  const focusIssue = (issue: string) => {
    const target = issue.includes('nhà cung cấp') ? { step: 0, id: 'receipt-supplier' } : issue.includes('ngày nhập') ? { step: 0, id: 'receipt-date' } : issue.includes('hóa đơn') ? { step: 0, id: 'receipt-invoice' } : issue.includes('ít nhất một SKU') ? { step: 1, id: 'receipt-content' } : { step: 2, id: 'receipt-content' }
    setStep(target.step); window.requestAnimationFrame(() => document.getElementById(target.id)?.focus())
  }
  const confirm = () => {
  ...
  return <div className="space-y-6"><Link to={ROUTES.warehouseReceipts} className="inline-flex items-center gap-2 text-sm font-semibold text-text-600"><ArrowLeft className="h-4 w-4" /> Danh sách phiếu nhập</Link><WarehousePageHeader eyebrow="Nhập kho" title={currentId || 'Tạo phiếu nhập kho'} description="Lưu nháp bất cứ lúc nào; tồn kho chỉ tăng sau bước xác nhận cuối." /><ProgressStepper steps={STEPS} currentStep={step} />
    <div aria-live="polite">{message && <p className="rounded-sm border border-success-200 bg-success-50 p-3 text-sm font-semibold text-success-700">{message}</p>}{issues.length > 0 && <div role="alert" className="rounded-sm border border-warning-200 bg-warning-50 p-4 text-sm text-warning-700"><strong>Còn {issues.length} vấn đề cần xử lý trước khi xác nhận:</strong><ul className="mt-2 list-disc space-y-1 pl-5">{issues.map((issue) => <li key={issue}><button type="button" className="text-left underline" onClick={() => focusIssue(issue)}>{issue}</button></li>)}</ul></div>}</div>
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px]"><section id="receipt-content" tabIndex={-1} className="rounded-md border border-surface-400 bg-white p-5 focus-visible:shadow-focus">
      {step === 0 && <div className="grid gap-4 md:grid-cols-2">
        <label className="text-sm font-semibold">Nhà cung cấp<input id="receipt-supplier" value={supplier} onChange={(event) => setSupplier(event.target.value)} className="input-gaming mt-2 w-full" /></label>
        <label className="text-sm font-semibold">Ngày nhập<input id="receipt-date" type="date" value={receiptDate} onChange={(event) => setReceiptDate(event.target.value)} className="input-gaming mt-2 w-full" /></label>
        <label className="text-sm font-semibold">Mã hóa đơn/chứng từ<input id="receipt-invoice" value={invoiceCode} onChange={(event) => setInvoiceCode(event.target.value)} className="input-gaming mt-2 w-full" /></label>
        <div className="text-sm font-semibold">
          <span>Ảnh minh chứng chứng từ</span>
          <div className="mt-2 flex flex-col gap-2">
            <input type="file" accept="image/*,.pdf" multiple className="text-xs text-text-600 file:btn-outlined file:mr-3 file:cursor-pointer" onChange={(e) => {
            ...
            {evidenceUrls.length > 0 && (
              <div className="flex flex-wrap gap-2 pt-1">
                {evidenceUrls.map((url, idx) => (
                  <span key={url} className="inline-flex items-center gap-1 rounded bg-surface-200 px-2 py-1 text-xs text-text-900">
                    Minh chứng #{idx + 1}
                    <button type="button" className="text-error-700 hover:underline" onClick={() => setEvidenceUrls((cur) => cur.filter((_, i) => i !== idx))}>×</button>
                  </span>
                ))}
              </div>
            )}
        ...
        <label className="text-sm font-semibold md:col-span-2">Ghi chú<textarea value={notes} onChange={(event) => setNotes(event.target.value)} rows={4} className="input-gaming mt-2 w-full" /></label>
      </div>}
      {step === 1 && <div className="space-y-5"><div className="flex flex-col gap-3 rounded-sm border border-info-200 bg-info-50 p-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-heading font-semibold">Chọn SKU cần nhập</h2><p className="mt-1 text-sm text-text-600">Mở danh sách riêng để tìm và kiểm tra đúng cấu hình.</p></div><button type="button" className="btn-primary" onClick={() => setSkuSelectorOpen(true)}>Chọn SKU</button></div>{lines.length === 0 ? <p className="text-sm text-text-600">Chưa có SKU trong phiếu.</p> : lines.map((line) => { const variant = store.variants.find((item) => item.id === line.variantId); const product = store.products.find((item) => item.id === variant?.productId); return <div key={line.id} className="flex items-center justify-between rounded-sm border border-surface-400 p-4"><div><p className="font-semibold">{product?.name}</p><p className="mt-1 font-mono text-xs">{variant?.sku} · {variant?.optionValues.map((item) => item.value).join(' / ')}</p></div><button type="button" aria-label={`Xóa ${variant?.sku}`} className="flex h-11 w-11 items-center justify-center rounded-sm border border-error-200 text-error-700 hover:bg-error-50" onClick={() => setLines((current) => current.filter((item) => item.id !== line.id))}><Trash2 className="h-4 w-4" /></button></div> })}</div>}
      {step === 2 && <div className="space-y-4">{lines.length === 0 ? <p className="text-sm text-text-600">Quay lại bước trước để thêm SKU.</p> : lines.map((line) => { const variant = store.variants.find((item) => item.id === line.variantId); const product = store.products.find((item) => item.id === variant?.productId); return <article key={line.id} className="rounded-sm border border-surface-400 p-4"><div className="flex flex-wrap justify-between gap-3"><div><p className="font-semibold">{product?.name}</p><p className="font-mono text-xs text-text-600">{variant?.sku}</p></div>{variant?.serialTracking && <StatusBadge label={line.serials.length === line.quantity ? 'Serial hợp lệ' : `Đã nhập ${line.serials.length}/${line.quantity}`} tone={line.serials.length === line.quantity ? 'success' : 'warning'} />}</div><div className="mt-4 grid gap-3 md:grid-cols-4"><label className="text-sm font-semibold">Số lượng<input type="number" min="1" step="1" value={line.quantity} onChange={(event) => setLines((current) => current.map((item) => item.id === line.id ? { ...item, quantity: Number(event.target.value) } : item))} className="input-gaming mt-2 w-full tabular-nums" /></label><label className="text-sm font-semibold">Giá nhập<input type="number" min="1" value={line.unitCost} onChange={(event) => setLines((current) => current.map((item) => item.id === line.id ? { ...item, unitCost: Number(event.target.value) } : item))} className="input-gaming mt-2 w-full tabular-nums" /></label><label className="text-sm font-semibold">Vị trí cất hàng<input placeholder="VD: Kệ A - Tầng 1" value={line.putawayLocation ?? ''} onChange={(event) => setLines((current) => current.map((item) => item.id === line.id ? { ...item, putawayLocation: event.target.value } : item))} className="input-gaming mt-2 w-full" /></label><div><p className="text-sm font-semibold">Thành tiền</p><p className="mt-4 font-semibold tabular-nums">{formatCurrency(line.quantity * line.unitCost)}</p></div></div>{variant?.serialTracking && <button type="button" className="btn-outlined mt-4" onClick={() => setEditingSerialLineId(line.id)}><ScanLine className="h-4 w-4" /> Nhập / Quét serial</button>}</article>})}</div>}
      {step === 3 && <div className="space-y-5"><div className="grid gap-3 md:grid-cols-2"><p><span className="text-xs text-text-600">Nhà cung cấp</span><strong className="block">{supplier || 'Chưa chọn'}</strong></p><p><span className="text-xs text-text-600">Ngày nhập</span><strong className="block">{receiptDate || 'Chưa chọn'}</strong></p><p><span className="text-xs text-text-600">Chứng từ</span><strong className="block">{invoiceCode || 'Chưa nhập'}</strong></p>{evidenceUrls.length > 0 && <p><span className="text-xs text-text-600">Minh chứng</span><strong className="block">{evidenceUrls.length} tệp đính kèm</strong></p>}</div><div className="overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><thead className="bg-surface-200"><tr><th className="p-3">SKU</th><th className="p-3">Vị trí cất</th><th className="p-3">Số lượng</th><th className="p-3">Giá nhập</th><th className="p-3">Serial</th></tr></thead><tbody>{lines.map((line) => { const variant = store.variants.find((item) => item.id === line.variantId); return <tr key={line.id} className="border-b border-surface-400"><td className="p-3 font-mono">{variant?.sku}</td><td className="p-3">{line.putawayLocation || '—'}</td><td className="p-3">{line.quantity}</td><td className="p-3">{formatCurrency(line.unitCost)}</td><td className="p-3">{variant?.serialTracking ? <button type="button" className="btn-outlined text-xs py-1 px-2 text-brand-500 border-brand-500" onClick={() => setViewingSerialLineId(line.id)}>Xem chi tiết ({line.serials.length}/{line.quantity})</button> : '—'}</td></tr> })}</tbody></table></div><button type="button" className="btn-primary" onClick={requestConfirm}>Xác nhận phiếu nhập</button></div>}
    ...
    <div className="flex justify-between"><button type="button" className="btn-outlined" disabled={step === 0} onClick={() => setStep((current) => Math.max(0, current - 1))}><ArrowLeft className="h-4 w-4" /> Quay lại</button>{step < STEPS.length - 1 && <button type="button" disabled={!stepCompletion[step]} title={!stepCompletion[step] ? 'Hoàn thành bước hiện tại để tiếp tục' : undefined} className="btn-primary disabled:cursor-not-allowed disabled:opacity-40" onClick={() => setStep((current) => Math.min(STEPS.length - 1, current + 1))}>Tiếp tục <ArrowRight className="h-4 w-4" /></button>}</div>
    <SkuSelectorDialog isOpen={skuSelectorOpen} products={store.products} variants={activeVariants} excludedVariantIds={lines.map((line) => line.variantId)} onCancel={() => setSkuSelectorOpen(false)} onConfirm={addVariant} />
    ...
    {viewingLine && viewingVariant && (
      <ConfirmDialog isOpen={true} title={`Chi tiết Serial: ${viewingVariant.sku}`} description={viewingLine.serials.length > 0 ? viewingLine.serials.join('\n') : 'Chưa có serial nào được nhập.'} confirmLabel="Đóng" cancelLabel="" onCancel={() => setViewingSerialLineId(null)} onConfirm={() => setViewingSerialLineId(null)} />
    )}
