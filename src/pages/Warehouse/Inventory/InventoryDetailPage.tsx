...
export function InventoryDetailPage() {
  ...
  const [skuPickerMode, setSkuPickerMode] = useState<'filter' | 'adjust' | null>(null)
  ...
  return <div className="space-y-6">
    ...
    <SkuSelectorDialog isOpen={skuPickerMode !== null} products={[product]} variants={skuPickerMode === 'adjust' ? quantityOnlyVariants : productVariants.filter((variant) => variant.serialTracking)} onCancel={() => setSkuPickerMode(null)} onConfirm={(variantId) => { if (skuPickerMode === 'adjust') setAdjustVariantId(variantId); else setParams((current) => { const next = new URLSearchParams(current); next.set('sku', variantId); return next }); setSkuPickerMode(null) }} />
  </div>
}
