...
export type OrderListTab = 'ALL' | 'WAITING_ACCEPTANCE' | 'PREPARING' | 'WAITING_GHTK_PICKUP' | 'COMPLETED' | 'ISSUE'

interface OrderTabConfig {
  key: OrderListTab
  label: string
  matches: (state: WarehouseOrderState) => boolean
}

const ORDER_TABS: OrderTabConfig[] = [
  { key: 'ALL', label: 'Tất cả', matches: () => true },
  { key: 'WAITING_ACCEPTANCE', label: 'Tiếp nhận', matches: (s) => s === 'WAITING_ACCEPTANCE' },
  { key: 'PREPARING', label: 'Chuẩn bị & Đóng gói', matches: (s) => s === 'PICKING' || s === 'WAITING_SERIAL' || s === 'READY_TO_PACK' },
  { key: 'WAITING_GHTK_PICKUP', label: 'Chờ Đơn Vị Vận Chuyển Lấy', matches: (s) => s === 'WAITING_GHTK_PICKUP' },
  { key: 'COMPLETED', label: 'Hoàn tất', matches: (s) => s === 'COMPLETED' },
  { key: 'ISSUE', label: 'Có sự cố', matches: (s) => s === 'ISSUE' },
]

function getFilterTabForState(state: WarehouseOrderState): OrderListTab {
  if (state === 'PICKING' || state === 'WAITING_SERIAL' || state === 'READY_TO_PACK') return 'PREPARING'
  if (state === 'WAITING_ACCEPTANCE') return 'WAITING_ACCEPTANCE'
  if (state === 'WAITING_GHTK_PICKUP') return 'WAITING_GHTK_PICKUP'
...
export function OrderListPage() {
  ...
  const activeTabKey = tab === 'PICKING' || tab === 'WAITING_SERIAL' || tab === 'READY_TO_PACK' ? 'PREPARING' : tab

  const rows = useMemo(
    () =>
      sortOrders(
        orders.filter((order) => {
          const matchesQuery = `${order.id} ${order.customerName}`.toLocaleLowerCase('vi').includes(debouncedQuery.trim().toLocaleLowerCase('vi'))
          const tabConfig = ORDER_TABS.find((item) => item.key === activeTabKey)
          const matchesStatus = tabConfig ? tabConfig.matches(order.state) : true
          const matchesPayment = !payment || order.paymentMethod === payment
          const matchesStaff = !staff || order.assignee === staff
          const matchesMine = !mine || order.assignee === CURRENT_STAFF
          return matchesQuery && matchesStatus && matchesPayment && matchesStaff && matchesMine
        }),
        sort
      ),
    [activeTabKey, debouncedQuery, mine, orders, payment, sort, staff]
  )
  ...
  return <div className="space-y-6"><WarehousePageHeader eyebrow="Outbound operations" title="Đơn hàng" description="Theo dõi và xử lý đơn hàng từ tiếp nhận, chuẩn bị đóng gói đến bàn giao vận chuyển." />
    <div className="flex gap-2 overflow-x-auto" role="tablist">
      {ORDER_TABS.map((tabItem) => {
        const isSelected = activeTabKey === tabItem.key
        const count = orders.filter((order) => tabItem.matches(order.state)).length
        return (
          <button
            key={tabItem.key}
            role="tab"
            ...
            type="button"
            className={isSelected ? 'btn-primary whitespace-nowrap' : 'btn-outlined whitespace-nowrap'}
            onClick={() => updateParam('status', tabItem.key)}
          >
            {tabItem.label} ({count})
          </button>
        )
