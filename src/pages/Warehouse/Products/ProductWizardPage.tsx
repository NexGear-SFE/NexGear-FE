...
import type { ProductSpecification, ProductVariant } from '@/types/product.type'
...
export function ProductWizardPage() {
  ...
  const { categories, products, variants, saveProduct } = useWarehouseStore()
  ...
  const effectiveVariants = useMemo(() => variantDrafts.map((variant) => ({ ...variant, sku: variant.sku || generateSkuPreview(product.brandCode, product.modelCode, variant.optionValues) })), [product.brandCode, product.modelCode, variantDrafts])
  ...
  const updateVariant = (index: number, patch: Partial<VariantDraft>) => { setVariantDrafts((current) => current.map((variant, itemIndex) => itemIndex === index ? { ...variant, ...patch } : variant)); setIsDirty(true) }

  function validateStep(index: number): boolean {
    setError('')
    if (index === 0) {
      const result = productSchema.safeParse(product)
      if (!result.success) { setError(result.error.issues[0]?.message ?? 'Thông tin cơ bản chưa hợp lệ.'); return false }
    }
    if (index === 1) {
      const keys = cleanSpecifications.map((item) => item.key.trim().toLocaleLowerCase('vi'))
      if (cleanSpecifications.some((item) => !item.key.trim() || !item.value.trim())) { setError('Mỗi thông số cần đủ tên và giá trị.'); return false }
      if (new Set(keys).size !== keys.length) { setError('Tên thông số không được trùng trong cùng sản phẩm.'); return false }
    }
    if (index === 2) {
      if (!effectiveVariants.length) { setError('Sản phẩm cần ít nhất một biến thể.'); return false }
      const result = variantCollectionSchema.safeParse(effectiveVariants)
      if (!result.success) { setError(result.error.issues[0]?.message ?? 'Biến thể/SKU chưa hợp lệ.'); return false }
      if (effectiveVariants.some((variant) => variants.some((item) => item.productId !== productId && item.sku === variant.sku))) { setError('SKU đã tồn tại ở sản phẩm khác.'); return false }
    }
    return true
  }
  ...
  return <div className="space-y-6">
    ...
    <div className="sticky top-20 z-10 rounded-md border border-surface-400 bg-white p-3 shadow-clay-sm"><ProgressStepper steps={steps} currentStep={step} /></div>
    <section className="card-gaming p-5 md:p-7">
      ...
      {step === 2 && <div><div className="mb-4 flex items-center justify-between"><div><h2 className="font-heading text-lg font-semibold">Biến thể và SKU</h2><p className="mt-1 text-sm text-text-600">Sản phẩm không có variation vẫn dùng một biến thể Default.</p></div><button type="button" className="btn-outlined" onClick={() => setVariantDrafts((current) => [...current, { sku: generateSkuPreview(product.brandCode, product.modelCode, [{ option: 'Variant', value: `Variant ${current.length + 1}`, code: `V${current.length + 1}` }]), skuSource: 'AUTO', optionValues: [{ option: 'Variant', value: `Variant ${current.length + 1}`, code: `V${current.length + 1}` }], barcode: '', gtin: '', serialTracking: false, reorderLevel: 0, status: 'ACTIVE', skuLocked: false }])}><Plus className="h-4 w-4" /> Thêm variant</button></div><div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead className="bg-surface-200 text-left text-xs uppercase text-text-600"><tr><th className="p-3">SKU</th><th className="p-3">Barcode</th><th className="p-3">GTIN</th><th className="p-3">Serial</th><th className="p-3">Reorder</th><th className="p-3">Trạng thái</th><th /></tr></thead><tbody>{variantDrafts.map((variant, index) => <tr key={index} className="border-b border-surface-400"><td className="p-3"><input aria-label={`SKU ${index + 1}`} value={variant.sku || generateSkuPreview(product.brandCode, product.modelCode, variant.optionValues)} onChange={(event) => updateVariant(index, { sku: normalizeSkuSegment(event.target.value), skuSource: 'MANUAL' })} className="input-gaming w-56 font-mono" /></td><td className="p-3"><input aria-label={`Barcode ${index + 1}`} value={variant.barcode ?? ''} onChange={(event) => updateVariant(index, { barcode: event.target.value })} className="input-gaming w-40" /></td><td className="p-3"><input aria-label={`GTIN ${index + 1}`} value={variant.gtin ?? ''} onChange={(event) => updateVariant(index, { gtin: event.target.value })} className="input-gaming w-40" /></td><td className="p-3"><input aria-label={`Serial tracking ${index + 1}`} type="checkbox" checked={variant.serialTracking} onChange={(event) => updateVariant(index, { serialTracking: event.target.checked })} className="h-5 w-5 accent-brand-500" /></td><td className="p-3"><input aria-label={`Reorder level ${index + 1}`} type="number" min="0" value={variant.reorderLevel} onChange={(event) => updateVariant(index, { reorderLevel: Number(event.target.value) })} className="input-gaming w-24" /></td><td className="p-3"><select aria-label={`Trạng thái SKU ${index + 1}`} value={variant.status} onChange={(event) => updateVariant(index, { status: event.target.value as VariantDraft['status'] })} className="input-gaming"><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></select></td><td><IconButton label="Xóa variant" onClick={() => setVariantDrafts((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Minus /></IconButton></td></tr>)}</tbody></table></div></div>}
      {step === 3 && <div className="grid gap-6 lg:grid-cols-2"><Summary title="Product master" rows={[['Tên', product.name], ['Product code', product.productCode], ['Model', product.modelCode], ['Brand', product.brand], ['Danh mục', categoryBreadcrumb], ['Trạng thái', product.status], ['Bảo hành', `${product.warrantyMonths} tháng`]]} /><Summary title="Biến thể / SKU" rows={variantDrafts.map((variant) => [variant.sku, `${variant.serialTracking ? 'Có' : 'Không'} serial tracking`])} /><Summary title="Thông số kỹ thuật" rows={cleanSpecifications.map((spec) => [spec.key, spec.value])} /><div className="rounded-md border border-warning-500 bg-amber-50 p-4 text-sm"><strong>Kiểm tra trước khi lưu</strong><p className="mt-2 text-text-600">Product cần category hợp lệ, specification không trùng và tối thiểu một SKU duy nhất.</p></div></div>}
      {error && <p role="alert" className="mt-5 rounded-sm bg-error-50 p-3 text-sm text-error-700">{error}</p>}
...
function IconButton({ children, label, onClick }: { children: ReactElement; label: string; onClick: () => void }) { return <button type="button" aria-label={label} onClick={onClick} className="flex h-11 w-9 items-center justify-center rounded-sm hover:bg-surface-200 [&_svg]:h-4 [&_svg]:w-4">{children}</button> }
