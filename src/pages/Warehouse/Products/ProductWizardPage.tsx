...
const steps = ['Thông tin sản phẩm', 'Thông số chung', 'Cấu hình & SKU', 'Kiểm tra']
...
export function ProductWizardPage() {
  ...
  const [variantDrafts, setVariantDrafts] = useState<VariantDraft[]>(() => {
    ...
    return current.length ? current : [{ sku: '', skuSource: 'AUTO', optionValues: [], barcode: '', gtin: '', purchasePrice: 0, serialTracking: false, reorderLevel: 0, status: 'ACTIVE', skuLocked: false }]
  })
  ...
  const stepCompletion = useMemo(() => {
    const specificationKeys = cleanSpecifications.map((item) => item.key.trim().toLocaleLowerCase('vi'))
    const specificationsValid = !cleanSpecifications.some((item) => !item.key.trim() || !item.value.trim()) && new Set(specificationKeys).size === specificationKeys.length
    const catalogSkus = variants.filter((item) => item.productId !== productId).map((item) => item.sku)
    const variantsValid = effectiveVariants.length > 0 && variantCollectionSchema.safeParse(effectiveVariants).success && findDuplicateSkus(effectiveVariants.map((variant) => variant.sku), catalogSkus).size === 0
    return [productSchema.safeParse(product).success, specificationsValid, variantsValid, true]
  }, [cleanSpecifications, effectiveVariants, product, productId, variants])
