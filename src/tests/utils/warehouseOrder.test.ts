...
describe('warehouse order utilities', () => {
  ...
  it('renders an issue at its resume step', () => expect(getOrderProgressIndex(initialOrders.find((order) => order.state === 'ISSUE')!, false)).toBe(1))
  it('maps fulfillment preparation states to index 1', () => expect(getOrderProgressIndex(initialOrders.find((order) => order.state === 'READY_TO_PACK')!, false)).toBe(1))
