...
describe('warehouse product lifecycle', () => {
  it('creates a draft with a default variant, activates and deactivates without deleting history', () => {
    const state = useWarehouseStore.getState()
    const id = state.saveProduct({ name: 'Test Product', slug: 'test-product', productCode: 'PTEST', modelCode: 'T1', brand: 'Test', brandCode: 'TST', categoryId: 'cat-mouse', shortDescription: '', specifications: [], warrantyMonths: 12, unit: 'Chiếc', origin: '', weightGrams: 0, dimensions: { lengthMm: 0, widthMm: 0, heightMm: 0 }, status: 'DRAFT' }, [{ sku: 'TST-T1', skuSource: 'AUTO', optionValues: [], purchasePrice: 100000, serialTracking: false, reorderLevel: 0, status: 'ACTIVE', skuLocked: false }])
  ...
  it('archives a locked variant instead of rewriting its movement history', () => {
    ...
    useWarehouseStore.getState().saveProduct({ name: product.name, slug: product.slug, productCode: product.productCode, modelCode: product.modelCode, brand: product.brand, brandCode: product.brandCode, categoryId: product.categoryId, shortDescription: product.shortDescription, specifications: product.specifications, warrantyMonths: product.warrantyMonths, unit: product.unit, origin: product.origin, weightGrams: product.weightGrams, dimensions: product.dimensions, status: product.status }, [{ sku: 'ASU-G16-NEW', skuSource: 'MANUAL', optionValues: [{ option: 'CPU', value: 'New', code: 'NEW' }], purchasePrice: 100000, serialTracking: true, reorderLevel: 3, status: 'ACTIVE', skuLocked: false }], product.id)
