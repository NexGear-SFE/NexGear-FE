...
export function SerialSelectorDialog({ initialSelectedIds = [], isOpen, onCancel, onConfirm, requirements, serials, variants }: Props) {
  ...
  function closeDialog() { setQuery(''); setSelectedIds(initialSelectedIds); onCancel() }
  function confirmSelection() { const ids = selectedIds; setQuery(''); onConfirm(ids) }
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 sm:items-center sm:p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeDialog() }}><section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="serial-selector-title" className="flex max-h-[92vh] w-full max-w-3xl flex-col rounded-t-lg border border-surface-400 bg-white shadow-clay-lg sm:rounded-lg">
    ...
    <footer className="flex items-center justify-between gap-3 border-t border-surface-400 p-4"><span className="text-sm text-text-600">Đã chọn {selectedIds.length} serial</span><div className="flex gap-2"><button type="button" className="btn-outlined" onClick={closeDialog}>Hủy</button><button type="button" disabled={!complete} className="btn-primary disabled:cursor-not-allowed disabled:opacity-40" onClick={confirmSelection}>Xác nhận serial</button></div></footer>
  </section></div>
}
