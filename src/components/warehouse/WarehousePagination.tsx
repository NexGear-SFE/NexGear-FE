...
export function WarehousePagination({ currentPage, onPageChange, pageSize, totalItems }: WarehousePaginationProps) {
  ...
  return <nav aria-label="Phân trang" className="flex flex-col gap-3 border-t border-surface-400 bg-white px-4 py-3 text-sm text-text-600 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div className="flex items-center gap-2">
      <button type="button" aria-label="Trang trước" disabled={safePage === 1} onClick={() => onPageChange(safePage - 1)} className="btn-outlined min-h-10 px-3 disabled:cursor-not-allowed disabled:opacity-50"><ChevronLeft className="h-4 w-4" /></button>
      <span aria-live="polite" className="min-w-20 text-center font-semibold text-text-900">{safePage} / {totalPages}</span>
      <button type="button" aria-label="Trang sau" disabled={safePage === totalPages} onClick={() => onPageChange(safePage + 1)} className="btn-outlined min-h-10 px-3 disabled:cursor-not-allowed disabled:opacity-50"><ChevronRight className="h-4 w-4" /></button>
    </div>
  </nav>
}
