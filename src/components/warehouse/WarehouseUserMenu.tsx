...
export function WarehouseUserMenu() {
  ...
  const logout = () => { window.localStorage.removeItem('warehouseSession'); navigate(ROUTES.home) }
  return <details className="group relative hidden sm:block">
    ...
    <div className="absolute right-0 top-[calc(100%+8px)] z-30 w-52 rounded-md border border-surface-400 bg-white p-1 shadow-clay-md">
      <div className="flex min-h-11 w-full items-center gap-2 rounded-sm px-3 text-sm"><UserRound className="h-4 w-4" /> Warehouse Staff</div>
      <button type="button" onClick={logout} className="flex min-h-11 w-full items-center gap-2 rounded-sm px-3 text-sm text-error-700 hover:bg-error-50 focus-visible:outline-none focus-visible:shadow-focus"><LogOut className="h-4 w-4" /> Đăng xuất</button>
    </div>
  </details>
}
