...
function OpenReceiptSerialDialog({ inventorySerials, onCancel, onSave, productName, quantity, serials, sku }: Omit<Props, 'isOpen'>) {
  ...
  useEffect(() => { previousFocusRef.current = document.activeElement as HTMLElement | null; window.setTimeout(() => inputRef.current?.focus(), 0); return () => previousFocusRef.current?.focus() }, [])
  useEffect(() => { const handleKeyDown = (event: KeyboardEvent) => { if (event.key === 'Escape') { onCancel(); return }; if (event.key !== 'Tab') return; const focusable = Array.from(dialogRef.current?.querySelectorAll<HTMLElement>('button:not([disabled]), textarea, [tabindex]:not([tabindex="-1"])') ?? []); const first = focusable[0]; const last = focusable.at(-1); if (!first || !last) return; if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last.focus() } else if (!event.shiftKey && document.activeElement === last) { event.preventDefault(); first.focus() } }; document.addEventListener('keydown', handleKeyDown); return () => document.removeEventListener('keydown', handleKeyDown) }, [onCancel])
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="presentation"><section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="serial-dialog-title" className="w-full max-w-2xl rounded-lg border border-surface-400 bg-white p-5 shadow-clay-lg"><div className="flex items-start justify-between gap-4"><div><h2 id="serial-dialog-title" className="font-heading text-lg font-semibold">Nhập serial sản phẩm</h2><p className="mt-1 text-sm text-text-600">{productName} · <span className="font-mono">{sku}</span> · cần {quantity} serial</p></div><button type="button" aria-label="Đóng nhập serial" className="btn-outlined px-3" onClick={onCancel}><X className="h-4 w-4" /></button></div>
